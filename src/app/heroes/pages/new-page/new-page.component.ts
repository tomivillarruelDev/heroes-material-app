...
export class NewPageComponent implements OnInit {
  ...
  onDeleteHero(): void {
    if ( !this.currentHero.id ) throw Error('The hero must have an id');
    const dialogRef = this.dialog.open( ConfirmDialogComponent , {
      data : this.heroForm.value })

      dialogRef.afterClosed()
        .pipe(
          filter( (resp: boolean) => resp ), //pasa solo si es true
          switchMap( () => this.heroesService.deleteHeroById( this.currentHero.id) ),
          filter( (wasDeleted: boolean) => wasDeleted ) //solamente pasa si es true osea si fue borrado
        )
        .subscribe( () => {
